...
module.exports = merge(base, {
  ...
  module: {
    rules: [
      { // 圖片資源
        ...
        options: {
          publicPath: `${customConfig.Release.sourcePublicPath}imgs/`,
          name: '[name].[ext]?[hash]',
          outputPath: 'imgs/'
        }
      },
      { // 字體資源
        ...
        options: {
          publicPath: `${customConfig.Release.sourcePublicPath}fonts/`,
          name: '[name].[ext]?[hash]',
          outputPath: 'fonts/'
        }
      },
  ...
  plugins: [
    new webpack.DefinePlugin({
      'process.env': {
        NODE_ENV: customConfig.Release.nodeEnv
      },
      'ROOT_FOLDER': customConfig.Release.rootFolder,
      'API_URL': customConfig.Release.apiUrl
    }),
